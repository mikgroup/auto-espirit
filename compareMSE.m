function [trueMSE, trueOptParam, sureFullMSE, sureFullOptParam, sureCalMSE, sureCalOptParam] = compareMSE(fileID, X, Y, r, weight, stdev, lst_k, lst_c, lst_wnsvn)
    ...
    for k = lst_k
        ...
        if (weight == true)
            [trueMSE(kdx, :), sureFullMSE(kdx, :), sureCalMSE(kdx, :)] = ...
                softMSE(A, r, k, lst_c, stdev, I, x, yf, yc, fileID);
        else
            [trueMSE(kdx, :, :), sureFullMSE(kdx, :, :), sureCalMSE(kdx, :, :)] = ...
              hardMSE(A, r, k, lst_c, lst_wnsvn, stdev, I, x, yf, yc, fileID);
        end
...
function [trueMSE, sureFullMSE, sureCalMSE] = pointMSE(M, W, r, c, stdev, I, x, yf, yc, fileID)
    ...
    for pdx=1:1:nc
        for qdx=1:1:nc
            T(:, :, pdx, qdx) = sum( ... 
                ...
                conj(ifft2c(zpad(extractCalreg(fft2c(squeeze(maps(:, :, qdx, :))), r), size(x)))), ... 
             3);
        end
    end 
...
function [trueMSE, sureFullMSE, sureCalMSE] = softMSE(A, r, k, lst_c, stdev, I, x, yf, yc, fileID)
    ...
    for c = lst_c
        ...
        cdx = cdx + 1;
        [trueMSE(cdx), sureFullMSE(cdx), sureCalMSE(cdx)] = pointMSE(M, W, r, c, stdev, I, x, yf, yc);
    end
end

function [trueMSE, sureFullMSE, sureCalMSE] = hardMSE(A, r, k, lst_c, lst_wnsvn, stdev, I, x, yf, yc, fileID)
    ...
    for w = lst_wnsvn
        ...
        for c = lst_c
            fprintf(fileID, '| | | c = %0.3f\n', c);
            cdx = cdx + 1;
            [trueMSE(cdx, wdx), sureFullMSE(cdx, wdx), sureCalMSE(cdx, wdx)] = pointMSE(M, W, r, c, stdev, I, x, yf, yc);
        end
    end
end
